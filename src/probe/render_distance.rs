...
pub(super) async fn run(host: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if seen.len() != INITIAL_CHUNKS {
        return Err(Box::new(ProbeError::Phase("render initial chunks")));
    }
    ...
    while seen.len() < TOTAL_CHUNKS {
        ...
        has_non_flat_surface |= batch.has_non_flat_surface;
        reject_duplicate_chunks(&seen, &batch.positions)?;
        seen.extend(batch.positions);
    }
    if seen.len() != TOTAL_CHUNKS {
        return Err(Box::new(ProbeError::Phase("render total chunks")));
    }
    if max_followup_batch > MAX_FOLLOWUP_CHUNKS {
        return Err(Box::new(ProbeError::Phase("render max batch")));
    }
    if max_followup_payload_bytes > MAX_FOLLOWUP_PAYLOAD_BYTES {
        return Err(Box::new(ProbeError::Phase("render max payload bytes")));
    }
    if !has_non_flat_surface {
        return Err(Box::new(ProbeError::Phase("render non-flat terrain")));
    }
    if followup_batches == 0 {
        return Err(Box::new(ProbeError::Phase("render counters")));
    }
    println!(
        "render-distance counters radius={RADIUS} initial={INITIAL_CHUNKS} total={} followup_batches={followup_batches} max_followup_batch={max_followup_batch} max_followup_payload_bytes={max_followup_payload_bytes}",
        seen.len()
    );
    Ok(())
}
...
fn reject_duplicate_chunks(
    ...
    if batch.iter().any(|pos| seen.contains(pos)) {
        return Err(Box::new(ProbeError::Phase("render duplicate chunk")));
    }
    Ok(())
}
