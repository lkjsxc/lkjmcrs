...
pub(super) async fn run(host: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    expect_empty_selected_slot(&mut first).await?;
    ...
    expect_empty_selected_slot(&mut second).await
}
...
async fn expect_empty_selected_slot(
    ...
    block_mutation::send_use_item_on_at(&mut client.stream, 31, base).await?;
    ...
    block_mutation::validate_ack(ack.data, 31)?;
    let update =
    ...
    block_mutation::validate_update_at(update.data, BlockPos::new(1, 80, 0), 0)
}
