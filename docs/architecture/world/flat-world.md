...
- Air above `y=79`.
- Spawn at `0, 80, 0`.
- Initial chunk-cache radius `2`, centered on spawn chunk `0,0`.
- Initial terrain batch contains `25` chunks in a `5x5` square.

## Rules

1. Terrain generation is deterministic by chunk coordinate.
2. Generated chunks are immutable until block mutation exists.
3. Spawn chunk generation derives directly from the advertised client radius.
4. First milestone may keep generated chunks in memory.
5. Persistence is not required until survival-core work begins.
